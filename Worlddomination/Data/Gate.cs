...
namespace Worlddomination.Data
{
    class Gate
    {
        ...
        public static List<string> Load( string file )
        {
            ...
            else if (file.Equals("Whitelist"))
            {
                file_path = @"C:\Users\luponix\Desktop\[TOHA HEAVY INDUSTRIES]\[C]DiscordBot\Worlddomination\Data\Whitelist.txt";
            }
        ...
        public static void AddAndSaveBanlist(string add)
        {
            List<string> current = Load("Streamers");
            bool result = false;
            foreach (string ele in current)
            {
                if (ele.Equals(add)) result = true;
            ...
            if (!result) current.Add(add);
            using (StreamWriter sw = File.CreateText(@"C:\Users\luponix\Desktop\[TOHA HEAVY INDUSTRIES]\[C]DiscordBot\Worlddomination\Data\Streamers.txt"))
            {
                foreach (string element in current)
                {
                    sw.WriteLine(element);
        ...
        public static void Save( string file ,List<string> list )
        {
            string file_path = "";
            ...
            {
                Console.WriteLine("ERROR Data.Gate.Save got called with an invalid location argument: "+ file);
                return;
            }
